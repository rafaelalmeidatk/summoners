...
class RiotApi {
  ...
  getSummonerLinkData = (regionCode, summonerId) => {
    const regionCodeLC = regionCode && regionCode.toLowerCase()
    return axios.get(`/link-data/${regionCodeLC}/${summonerId}`)
  }

  getSummonerPageData = (baseUrl, regionCode, summonerName) => {
    const name = encodeURI(summonerName)
    const code = regionCode && regionCode.toLowerCase()
    return axios.get(`${baseUrl}/summoner-page/${code}/${name}`)
  }
}
